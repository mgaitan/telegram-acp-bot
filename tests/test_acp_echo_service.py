...
def test_permission_policy_defaults_and_updates(tmp_path: Path) -> None:
    ...
    assert asyncio.run(service.set_session_permission_mode(chat_id=1, mode="approve"))
    assert asyncio.run(service.set_next_prompt_auto_approve(chat_id=1, enabled=True))
